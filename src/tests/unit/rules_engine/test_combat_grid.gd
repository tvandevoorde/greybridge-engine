...
func _test_get_position() -> void:
	...
	var grid := CombatGridClass.new()
	_check(grid.get_position("nobody") == CombatGridClass.INVALID_POSITION, "unknown combatant returns INVALID_POSITION")
...
func _test_feet_per_tile_constant() -> void:
	...
	_check(CombatGridClass.FEET_PER_TILE == 5, "1 tile equals 5 feet")
