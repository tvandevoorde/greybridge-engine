...
const SpellEffectClass  = preload("res://rules_engine/core/spell_effect.gd")
...
func _test_make_attack_keys() -> void:
	...
	var effect: Dictionary = SpellEffectClass.make_attack(2, 6, 3, "fire")
	_check(effect["type"] == SpellEffectClass.TYPE_ATTACK, "type is TYPE_ATTACK")
...
func _test_make_saving_throw_keys() -> void:
	...
	var effect: Dictionary = SpellEffectClass.make_saving_throw(14, "DEX", 8, 6, 0, "fire", true)
	_check(effect["type"] == SpellEffectClass.TYPE_SAVING_THROW, "type is TYPE_SAVING_THROW")
...
func _test_make_healing_keys() -> void:
	...
	var effect: Dictionary = SpellEffectClass.make_healing(1, 8, 4)
	_check(effect["type"] == SpellEffectClass.TYPE_HEALING, "type is TYPE_HEALING")
...
func _test_make_concentration_keys() -> void:
	...
	var effect: Dictionary = SpellEffectClass.make_concentration(10)
	_check(effect["type"] == SpellEffectClass.TYPE_CONCENTRATION, "type is TYPE_CONCENTRATION")
...
func _test_attack_miss_deals_no_damage() -> void:
	...
	var executor := SpellExecutorClass.new()
...
func _test_attack_natural_20_critical_doubles_dice() -> void:
	...
	var effect: Dictionary = SpellEffectClass.make_attack(1, 4, 0, "force")
	var roller := DiceRollerClass.new(0)
	var executor := SpellExecutorClass.new()
...
func _test_attack_natural_1_auto_miss() -> void:
	...
	var effect: Dictionary = SpellEffectClass.make_attack(2, 6, 5, "cold")
	var roller := DiceRollerClass.new(0)
	var executor := SpellExecutorClass.new()
...
func _test_attack_result_keys() -> void:
	...
	var effect: Dictionary = SpellEffectClass.make_attack(1, 6, 0, "force")
	var roller := DiceRollerClass.new(0)
	var executor := SpellExecutorClass.new()
...
func _test_attack_damage_type_forwarded() -> void:
	...
	var effect: Dictionary = SpellEffectClass.make_attack(1, 6, 0, "lightning")
	var roller := DiceRollerClass.new(0)
	var executor := SpellExecutorClass.new()
	var result: Dictionary = executor.execute_attack(effect, 15, 0, 0, 10, roller)
...
func _test_saving_throw_result_keys() -> void:
	...
	var effect: Dictionary = SpellEffectClass.make_saving_throw(14, "DEX", 2, 6, 0, "fire", true)
	var roller := DiceRollerClass.new(0)
	var executor := SpellExecutorClass.new()
...
func _test_saving_throw_success_half_damage() -> void:
	...
	var effect: Dictionary = SpellEffectClass.make_saving_throw(10, "CON", 2, 6, 0, "cold", true)
	var roller := DiceRollerClass.new(0)
	var executor := SpellExecutorClass.new()
	var result: Dictionary = executor.execute_saving_throw(
...
func _test_saving_throw_success_no_half_when_flag_false() -> void:
	...
	var effect: Dictionary = SpellEffectClass.make_saving_throw(10, "DEX", 2, 6, 0, "fire", false)
	var roller := DiceRollerClass.new(0)
	var executor := SpellExecutorClass.new()
	var result: Dictionary = executor.execute_saving_throw(
		effect, func() -> int: return 15, 0, 0, false, roller
...
func _test_saving_throw_half_damage_rounds_down() -> void:
	...
	var effect: Dictionary = SpellEffectClass.make_saving_throw(8, "CON", 1, 20, 0, "necrotic", true)
...
func _test_healing_result_key() -> void:
	...
	var effect: Dictionary = SpellEffectClass.make_healing(1, 8, 0)
	var roller := DiceRollerClass.new(0)
	var executor := SpellExecutorClass.new()
...
func _test_healing_modifier_added() -> void:
	...
	var effect: Dictionary = SpellEffectClass.make_healing(1, 4, 4)
	var roller := DiceRollerClass.new(0)
	var executor := SpellExecutorClass.new()
	var result: Dictionary = executor.execute_healing(effect, roller)
...
func _test_concentration_dc_minimum_ten() -> void:
	...
	_check(SpellExecutorClass.concentration_dc(0) == 10,  "0 damage → DC 10")
	_check(SpellExecutorClass.concentration_dc(1) == 10,  "1 damage → DC 10 (floor(1/2)=0 < 10)")
	_check(SpellExecutorClass.concentration_dc(10) == 10, "10 damage → DC 10 (floor(10/2)=5 < 10)")
	_check(SpellExecutorClass.concentration_dc(18) == 10, "18 damage → DC 10 (floor(18/2)=9 < 10)")
	_check(SpellExecutorClass.concentration_dc(19) == 10, "19 damage → DC 10 (floor(19/2)=9 < 10)")
...
func _test_concentration_dc_above_minimum() -> void:
	...
	_check(SpellExecutorClass.concentration_dc(20) == 10, "20 damage → DC 10 (floor(20/2)=10 == 10)")
	_check(SpellExecutorClass.concentration_dc(22) == 11, "22 damage → DC 11 (floor(22/2)=11)")
	_check(SpellExecutorClass.concentration_dc(30) == 15, "30 damage → DC 15")
	_check(SpellExecutorClass.concentration_dc(50) == 25, "50 damage → DC 25")
...
func _test_concentration_dc_rounds_down() -> void:
	...
	_check(SpellExecutorClass.concentration_dc(21) == 10, "21 damage → DC 10 (floor(21/2)=10)")
	_check(SpellExecutorClass.concentration_dc(23) == 11, "23 damage → DC 11 (floor(23/2)=11)")
